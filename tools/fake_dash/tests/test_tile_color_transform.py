...
def derived_int_matrix() -> list[int]:
    ...
    constant folded into the alpha column (×256). Mirrors the Swift
    derivation comment exactly."""
...
def apply_matrix(px, matrix, divisor):
    ...
    vImage: rounded integer divide, clamp to [0, 255]. Channel-memory
    order is R, G, B, A (premultipliedLast on little-endian iOS)."""
    r, g, b, a = px
    src = (r, g, b, a)
    out = []
    half = divisor // 2
    for i in range(4):
        acc = sum(matrix[i * 4 + j] * src[j] for j in range(4))
...
class TestColorMatrixCoefficients(unittest.TestCase):
    ...
    def test_matrix_matches_w3c_derivation(self):
        ...
        re-derive from the W3C spec and confirm they match within the
        rounding of the ×256 fixed-point scale."""
        derived = derived_int_matrix()
        for shipped, ref in zip(MATRIX, derived):
            self.assertLessEqual(
                abs(shipped - ref), 1,
                f"coefficient drift: shipped {shipped} vs derived {ref}",
            )

    def test_alpha_row_is_identity(self):
        """Opacity must be preserved — alpha row passes A through."""
        self.assertEqual(MATRIX[12:16], [0, 0, 0, 256])

    def test_grey_axis_rows_are_consistent(self):
        """A neutral grey (v,v,v) must map to a neutral grey. That holds
        iff all three RGB rows act IDENTICALLY on the grey axis: equal
        linear row-sums AND equal alpha-column constants. Here every RGB
        row sums to -divisor (the `-HueRotate` linear part; HueRotate rows
        sum to 1.0) and every alpha column is +divisor (the folded invert
        constant), so each row computes 255-v -> grey in, grey out."""
        row_sums = [
            MATRIX[i * 4] + MATRIX[i * 4 + 1] + MATRIX[i * 4 + 2]
            for i in range(3)
        ]
        # All three identical -> no hue cast on the grey axis.
        self.assertEqual(len(set(row_sums)), 1, f"grey-axis cast: {row_sums}")
        # And equal to -divisor, i.e. the invert maps v -> 255 - v.
        self.assertEqual(row_sums[0], -DIVISOR)
        # Alpha columns of the three RGB rows all carry the same constant.
        alpha_cols = [MATRIX[i * 4 + 3] for i in range(3)]
        self.assertEqual(set(alpha_cols), {DIVISOR})
