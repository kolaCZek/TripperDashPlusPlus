...
Two fixes, guarded here:
  ...
  2. AddBikeSheet must validate the SSID against the Tripper AP format
     `RE_XXXX_XXXXX` so an IP / home-Wi-Fi name can't be saved as a bike.
...
TRIPPER_SSID_RE = re.compile(r"^RE_[A-Z0-9]{4}_[A-Z0-9]{5,6}$")
...
def test_swift_regex_matches_mirror():
    ...
    src = _add_bike_sheet_src()
    assert r"^RE_[A-Z0-9]{4}_[A-Z0-9]{5,6}$" in src, (
...
def test_valid_tripper_ssids_pass():
    for s in ("RE_DEMO_000001", "RE_0W12_34567", "RE_XXXX_XXXXX", "RE_ABCD_98765"):
        assert TRIPPER_SSID_RE.match(s), f"{s} should be a valid Tripper SSID"


def test_ip_and_home_names_rejected():
    for s in ("192.168.1.1", "MYHOMEWIFI", "RE_ABC_12345", "RE_DEMO_0001", "TRIPPER"):
