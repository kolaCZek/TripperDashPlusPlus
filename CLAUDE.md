...
- **Distribution**: Paid Apple Developer Program — required for the **Hotspot Configuration** entitlement (`com.apple.developer.networking.HotspotConfiguration`), which powers the in-app Wi-Fi auto-join to the dash AP (see `WiFiJoiner.swift` / `TripperDashPP.entitlements`). A free Personal Team cannot sign the app. The rest of the app is deliberately built keyless (OSM tiles, Overpass, Open-Meteo, CallKit instead of push).
...
**Background execution**: the app is designed to run with the screen locked / phone in a pocket. The map frame source pre-renders OSM tiles while the app is foregrounded (GPU awake), then in the background does **CPU-only CGContext composition** (crop tile around current GPS fix, rotate heading-up, draw polyline + heading chevron) — CGContext is background-safe where MapKit's Metal renderer and `MKMapSnapshotter` are not. The render loop is kept alive by **background `CoreLocation` updates** (`allowsBackgroundLocationUpdates = true`, `pausesLocationUpdatesAutomatically = false`) as the single, self-sufficient wakelock. The flag is set on both Always and While Using: updates are always started from a foreground action (connect / start ride), and a While Using app that started them in the foreground keeps receiving them with the screen locked (blue location pill). Always is still requested, but While Using is enough to ride. (Two earlier belt-and-braces wakelocks were removed: the AVKit Picture-in-Picture anchor in Phase 8d, and the `SilentAudioKeeper` silent-audio loop before submission — a silent loop purely to stay alive is an App Review red flag. The `audio` background mode stays, now backed by a real audio feature: `VoiceNavigator` spoken turn-by-turn guidance. Don't reintroduce either wakelock; the tile-cache + CGContext path is BG-safe on background location alone.)
...
# 1. Signing & Capabilities → Team = a paid Apple Developer Program team, Bundle ID = unique (e.g. eu.YOURNAME.TripperDashPP)
...
- Don't add a new dependency (SPM package) without flagging it in a PR description with justification. Each dep is a future migration burden. The app currently has **zero** third-party SPM dependencies — keep it that way unless there's a strong reason.
